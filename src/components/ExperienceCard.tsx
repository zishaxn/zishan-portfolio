  ...
  return (
    <div className="flex gap-6">
      <div className="flex flex-col items-center">
        <div className="w-3 h-3 rounded-full border-2 border-[#4f7cff] bg-[#0f1115] shrink-0 mt-1" />
      ...
      <div className={`pb-12 flex-1 ${isLast ? "pb-0" : ""}`}>
        ...
        <p className="text-sm text-[#4f7cff] mb-4">{company}</p>
        ...
        <ul className="space-y-2 mb-5">
          {highlights.map((h) => (
            <li key={h} className="text-sm text-[#9ca3af] flex items-start gap-2">
              <span className="text-[#4f7cff] shrink-0 mt-0.5">›</span>
              {h}
            </li>
          ))}
        </ul>
